...
Keystone is a **local credential proxy and authenticated request tunnel** for one extension-first integration.

In v1, it is built for the extension integration that needs it now. The first consumer is the extension. The host may be extensible internally, but the product is not presented as a multi-extension platform, local database, or generic automation runtime.
...
The most honest framing is: **local credential proxy and authenticated request tunnel**.
Avoid grander terms like key guard, hard security boundary, or generic local runtime.
...
- Start with Native Messaging only if that is the fastest way to prove the extension integration.
...
1. The extension opens a Native Messaging connection.
2. Keystone reads the caller's extension identity from the Native Messaging context.
3. If the extension is unknown, Keystone shows a clear local approval prompt naming the extension and its requested provider access.
...
In the extension-first v1, there is exactly one supported paired extension. The storage model can support more later, but the product and UX do not.
...
1. The paired extension calls `llm.open_session` over Native Messaging for a named provider.
...
4. The extension uses that token as a Bearer token on localhost HTTP requests.
...
V1 should support only the providers the extension client actually needs first. Do not broaden provider support for marketing symmetry.
...
- The extension sends provider-native request bodies. Keystone does not normalize or translate between incompatible provider APIs in v1.
- The extension does not choose arbitrary upstream URLs at request time. Provider and route selection remain bound to server-side configuration and session scope.
...
- Pairing for the first extension
- Short-lived in-memory sessions
- Localhost HTTP forwarding on a random loopback port
- `POST /v1/chat/completions` passthrough for the first required extension endpoint
- Streaming passthrough if the extension needs streaming
- URL allowlist validation per provider
- Minimal diagnostics: configured providers, pairing state, recent request log

The first shipping milestone does not need a dedicated desktop UI beyond what is necessary for pairing and basic configuration. A full settings shell or tray app can follow after the host protocol and extension flow are stable.

## Non-Goals

These are out of scope for v1 and should not appear in implementation planning:

- Generic local storage for extension feature data
...
- Better provider coverage only when driven by the extension
...
- Optional local model adapters later if they fit the same credential-proxy pattern

Healthy extensibility means reusable internal modules. Unhealthy extensibility means promising a local platform before the credential-proxy flow proves itself in real extension use.

## Final Definition

Keystone is a local credential proxy for a browser extension that needs to call provider APIs without keeping long-lived provider credentials in browser storage.
...
Local-first. No cloud relay. Browser-extension-first scope.

## SECTION 2 — DESIGN DECISIONS AND REJECTED ALTERNATIVES

### Decisions

- **Smaller, honest product framing.** The product should stay close to credential proxying and request tunneling, not wider platform claims.
...
- **Extension-first v1.** The first version is scoped to one real extension integration. Data structures can support more later, but the product should not.
...
- **Large v1 desktop shell requirements.** Rejected because they delay the extension integration that validates the concept.
- **Provider schema normalization in the proxy.** Rejected because it broadens the host into a provider abstraction layer before the first extension flow is proven.
...
- The critique was right that shared generic infrastructure is a risk before the first extension proves the flow.
...
- OS credential store integration
- Local trust record store for the paired extension
- In-memory session store with TTL
- Narrow provider forwarding layer with explicit URL allowlists

If the fastest path to a working extension integration is Native Messaging-only, build that first. Treat it as the proving step, not the final architecture.
...
1. Extension sends `bridge.hello`.
2. Host checks for an existing trust record by extension ID.
3. If absent, extension sends `bridge.pair`.
4. Host presents a local approval UI that names the extension and requested provider scope.
...
1. Extension sends `llm.open_session` with a provider ID.
...
6. Extension sends localhost requests with `Authorization: Bearer <token>`.
...
The installer must write a Chrome Native Messaging host manifest whose `allowed_origins` contains only the intended extension ID for the first extension integration.
...
- The extension does not choose arbitrary upstream targets once a provider session is issued.
...
- Extension treats `401` or connection failure as a signal to re-run `bridge.hello` and `llm.open_session`.
...
- host manifest missing or `allowed_origins` not matching the extension ID
...
- port bind failure
- host restart during an active extension session
...
Each failure must produce a clear machine-readable error and a user-facing recovery path in the extension.
...
Add a small UI module only after the host protocol and extension integration are working end to end.

## Build This First

1. Native Messaging host skeleton with `bridge.hello`.
2. OS credential store wrapper for one provider.
3. Pairing flow for the first extension and persisted trust record.
...
7. End-to-end extension request against a real provider.
